class FriendshipController < ApplicationController
  ...
  def accept
    if @user.requested_friends.include?(@friend)
      ...
      flash[:notice] = "Frienship with #{@friend.screen_name} accepted!"
    else
  ...
  def decline
    if @user.requested_friends.include?(@friend)
      ...
      flash[:notice] = "Frienship with #{@friend.screen_name} declined"
    else
      flash[:notice] = "No friendship request from #{@friend.screen_name}."
    end
    redirect_to hub_url
  ...
  def cancel
    if @user.pending_friends.include?(@friend)
      Friendship.breakup(@user, @friend)
      flash[:notice] = "Frienship request cancelled."
    else
  ...
  def delete
    if @user.friends.include?(@friend)
      Friendship.breakup(@user, @friend)
      flash[:notice] = "Frienship with #{@friend.screen_name} deleted!"
    else
