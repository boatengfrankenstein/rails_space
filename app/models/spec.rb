class Spec < ActiveRecord::Base
  ...
  validates_inclusion_of :birthdate, :in => VALID_DATES,
                                     :allow_nil => true,
                                     :message => "is invalid"
  ...
  def self.find_by_asl(params)
    ...
    # Setup the gender restriction in SQL
  ...
  def self.sql_distance_away(point)
    h = "POWER(SIN((RADIANS(lattitude - #{point.lattitude}))/2.0),2) + " +
        "COS(RADIANS(#{point.lattitude})) * COS(RADIANS(lattitude)) * " +
